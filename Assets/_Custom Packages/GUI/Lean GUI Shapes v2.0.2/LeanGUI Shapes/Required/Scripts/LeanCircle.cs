...
namespace Lean.Gui
{
	...
	public class LeanCircle : MaskableGraphic
	{
		...
		public int Detail { set { if (detail != value) { detail = value; SetAllDirty(); } } get { return detail; } }
		[SerializeField] int detail = 10;

		/// <summary>This allows you to control where the triangle begins in degrees.
		/// 0 = Top.
		/// 90 = Right.</summary>
		public float Angle { set { if (angle != value) { angle = value; SetAllDirty(); } } get { return angle; } }
		...
		public float Fill { set { if (fill != value) { fill = value; SetAllDirty(); } } get { return fill; } }
		[SerializeField] float fill = 1.0f;

		static Texture2D blurTexture;
		static bool blurTextureSet;

		static UIVertex innerVert = UIVertex.simpleVert;
		static UIVertex outerVert = UIVertex.simpleVert;

		static VertexHelper vh;
		...
		static List<int> tailIndices = new List<int>();
		static List<int> edgeIndices = new List<int>();
		...
		void WriteEdge(List<int> indices)
		{
			if (indices.Count > 0)
			{
				edgeIndices.Clear();
