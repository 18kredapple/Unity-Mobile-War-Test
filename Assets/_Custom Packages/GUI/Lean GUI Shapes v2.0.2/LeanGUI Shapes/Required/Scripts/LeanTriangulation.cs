...
namespace Lean.Gui
{
	...
	public static class LeanTriangulation
	{
		...
		static bool CanClip(Vector2 pointA, Vector2 pointB, Vector2 pointC, int min, int max)
		{
		...
		static void Submit(int indexA, int indexB, int indexC)
		{
		...
		static float PointInSphere(Vector2 aVec, Vector2 bVec, Vector2 cVec, Vector2 dVec)
		{
		...
		static double LineSide(Vector2 a, Vector2 b, Vector2 p)
		{
			return (b.y - a.y) * (p.x - a.x) - (b.x - a.x) * (p.y - a.y);
		}

		static bool PointInTriangle(Vector2 a, Vector2 b, Vector2 c, Vector2 p) // NOTE: CW
		{
