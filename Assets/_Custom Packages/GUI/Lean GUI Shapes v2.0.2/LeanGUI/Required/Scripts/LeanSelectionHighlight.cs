...
namespace Lean.Gui
{
	...
	public class LeanSelectionHighlight : MonoBehaviour
	{
		...
		[SerializeField] LeanPlayer showTransitions;
		...
		[SerializeField] LeanPlayer hideTransitions;

		/// <summary>This allows you to perform an action when the highlight starts.</summary>
		public UnityEvent OnShow { get { if (onShow == null) onShow = new UnityEvent(); return onShow; } }
		...
		bool UpdateRect(RectTransform target)
		{
		...
		static Vector3 WorldToViewportPoint(Camera camera, Vector3 point, bool worldSpace)
		{
			if (worldSpace == false)
			{
