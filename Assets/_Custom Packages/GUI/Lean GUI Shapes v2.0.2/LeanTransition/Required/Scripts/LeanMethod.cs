...
namespace Lean.Transition
{
	...
	public abstract class LeanMethod : MonoBehaviour
	{
		...
		static float SmoothQuad(float x)
		{
			return x * x;
		}

		static float SmoothCubic(float x)
		{
			return x * x * x;
		}

		static float SmoothQuart(float x)
		{
			return x * x * x * x;
		}

		static float SmoothQuint(float x)
		{
			return x * x * x * x * x;
		}

		static float SmoothExpo(float x)
		{
			return x == 0.0f ? 0.0f : Mathf.Pow(2.0f, 10.0f * x - 10.0f);
		}

		static float SmoothCirc(float x)
		{
			return 1.0f - Mathf.Sqrt(1.0f - Mathf.Pow(x, 2.0f));
		}

		static float SmoothBack(float x)
		{
			return 2.70158f * x * x * x - 1.70158f * x * x;
		}

		static float SmoothElastic(float x)
		{
			return x == 0.0f ? 0.0f : x == 1.0f ? 1.0f : -Mathf.Pow(2.0f, 10.0f * x - 10.0f) * Mathf.Sin((x * 10.0f - 10.75f) * ((2.0f * Mathf.PI) / 3.0f));
		}

		static float SmoothBounce(float x)
		{
			if (x < (4f / 11f))
			{
				return (121f / 16f) * x * x;
...
namespace Lean.Transition.Editor
{
	...
	{
		static List<LeanMethod> tempMethods = new List<LeanMethod>();
		...
		void DrawTargetAlias(SerializedProperty sTarget, SerializedProperty sAlias, System.Reflection.FieldInfo data)
		{
		...
		void DrawAutoFill(TARGET tgt, System.Reflection.FieldInfo data)
		{
			var state = data.GetValue(tgt) as LeanState;
