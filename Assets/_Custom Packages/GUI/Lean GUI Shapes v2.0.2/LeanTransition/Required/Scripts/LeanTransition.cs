...
namespace Lean.Transition
{
	...
	public class LeanTransition : MonoBehaviour
	{
		...
		public LeanTiming DefaultTiming { set { defaultTiming = value; } get { return defaultTiming; } }
		[SerializeField][UnityEngine.Serialization.FormerlySerializedAs("Timing")] LeanTiming defaultTiming = LeanTiming.UnscaledUpdate;
		...
		static List<LeanState> unscaledUpdateStates = new List<LeanState>();

		static List<LeanState> unscaledLateUpdateStates = new List<LeanState>();

		static List<LeanState> unscaledFixedUpdateStates = new List<LeanState>();

		static List<LeanState> updateStates = new List<LeanState>();

		static List<LeanState> lateUpdateStates = new List<LeanState>();

		static List<LeanState> fixedUpdateStates = new List<LeanState>();

		static List<LeanMethod> tempBaseMethods = new List<LeanMethod>();

		static List<LeanMethod> baseMethodStack = new List<LeanMethod>();

		static Dictionary<string, System.Type> aliasTypePairs = new Dictionary<string, System.Type>();

		static LeanState previousState;

		static LeanState currentQueue;

		static LeanState defaultQueue;

		static LeanTiming currentTiming;

		static float currentSpeed = 1.0f;

		static bool started;

		static Dictionary<string, Object> currentAliases = new Dictionary<string, Object>();
		...
		static void AddAliasTypePairs(Transform root)
		{
			if (root != null)
			{
		...
		void RemoveConflictsBefore(List<LeanState> states, LeanState currentState, int currentIndex)
		{
		...
		void UpdateAll(List<LeanState> states, float delta)
		{
			ResetState();
		...
		static void FinishState(LeanState state)
		{
		...
		static void DirtyTarget(LeanState transition)
		{
			if (Application.isPlaying == false)
			{
