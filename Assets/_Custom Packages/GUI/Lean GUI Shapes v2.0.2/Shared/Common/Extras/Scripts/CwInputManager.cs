...
namespace CW.Common
{
	...
	public class CwInputManager : MonoBehaviour
	{
		...
		[SerializeField] LayerMask guiLayers = 1 << 5;
		...
		static List<RaycastResult> tempRaycastResults = new List<RaycastResult>(10);

		static PointerEventData tempPointerEventData;

		static EventSystem tempEventSystem;

		static List<Finger> fingers = new List<Finger>();

		static List<Finger> filteredFingers = new List<Finger>();

		static Stack<Finger> pool = new Stack<Finger>();
		...
		{
			if (Application.isPlaying == true && FindFirstObjectByType<CwInputManager>() == null)
			{
		...
		void AddFinger(int index, Vector2 screenPosition, float pressure, bool set)
		{
		...
		static Vector2 Hermite(Vector2 a, Vector2 b, Vector2 c, Vector2 d, float t)
		{
			var mu2 = t * t;
			var mu3 = mu2 * t;
		...
		static float HermiteInterpolate(float y0, float y1, float y2, float y3, float mu, float mu2, float mu3)
		{
		...
		static float GetRadians(Vector2 screenPosition, Vector2 referencePoint)
		{
			return Mathf.Atan2(screenPosition.x - referencePoint.x, screenPosition.y - referencePoint.y);
		}

		static float GetDeltaRadians(Finger finger, Vector2 referencePoint, Vector2 lastReferencePoint)
		{
